...
#else
#  include <x86intrin.h>
#  define HRD_ALWAYS_INLINE __attribute__((HRD_ALWAYS_INLINE))
...
#pragma pack(push, 4)
    ...
    struct StorageItem
    {
        StorageItem(StorageItem&& r) : data(std::move(r.data)) { mark = r.mark; /*set mark after data-ctor call, if any exception happens nothing break old mark value*/ }
